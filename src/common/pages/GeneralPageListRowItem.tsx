...
export default function GeneralPageListRowItem({
  ...
  const handleDeleteDocument = (id: number) => {
    const deleteTopic = async () => {
      try {
        ...
        handleReload();
        toast.success(t("message.success.deleted_doc"));
      } catch (error) {
        toast.success(t("message.error.deleted_doc"));
      }
    };
    setModal(
      <ConfirmModal
  ...
  const handleSearchData = async (params: Params) => {
    try {
      setLoading(true);
      const { total, data } = await topicService.searchTopics(params);
      setListData(data);
  ...
  useEffect(() => {
    if (keySearch !== "") handleSearchData(param);
  }, [currentPage]);
