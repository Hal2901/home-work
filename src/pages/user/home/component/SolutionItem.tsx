...
import { ElipSolution, SolutionCard } from "../../../../assets/images";
...
const SolutionItem = ({ item, reverse, index }: Props) => {
  ...
  return (
    <div className="w-full sc1800:px-300 xl:px-[155px] sm:px-100 px-5 grid lg:grid-cols-5 grid-cols-1 lg:gap-12 gap-y-10 relative lg:pb-20 lg:mb-16 mb-10">
      ...
      >
        <img
          src={getUrlImage(item.link!) ?? SolutionCard}
          alt=""
      ...
      >
        ...
        <p
          className={clsx(
            "xs:text-[32px] text-2xl leading-[40px] font-semibold",
            {
              "text-right": reverse,
            }
          )}
      ...
      >
        <img src={ElipSolution} alt="" className="" />
      </div>
    </div>
  );
};
